    ...
    <div className={styles.container}>
      <div className={styles.markscards}>
        {cards.map((card) => (
          <div
            key={card.id}
            className={`${styles.card} ${card.status ? styles[card.status] : ''}`}
          >
            <h3>{card.title}</h3>
            ...
            {card.status && <p className={styles.status}>{card.status === 'pass' ? 'Pass' : 'Fail'}</p>}
          </div>
        ))}
      </div>
    </div>
